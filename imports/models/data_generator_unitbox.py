...
class DataGenerator(keras.utils.Sequence):
    ...
    def __init__(self, df,df_labels,hist_equal=False, augment_data=False, batch_size=2, target_size=SHAPE, shuffle=True,save_images=False, input_channels=3):
        'Initialization'
        self.df = df
    ...
    def __getitem__(self, index):
        ...
        imgs, l_scores,l_bboxes  = self.__data_generation(tmp_df)
        #imgs = self.preprocess_input(imgs)
        return imgs,[l_scores,l_bboxes]
    ...
    def __get_bboxes(self,index):
        sr = self.df_labels[self.df_labels['imageFilename'].str.contains(str(index))]
        tmp = list(sr.values)[0][1:-1]
        chunks = [tmp[i:i + 4] for i in range(0, len(tmp), 4)]
        not_zero_chunks = [c for c in chunks if sum(c)>0]
        return not_zero_chunks
    ...
    def __data_generation(self, tmp_df):
        ...
        l_scores = np.empty((self.batch_size, *self.target_size, 1))
        l_bboxes = np.empty((self.batch_size, *self.target_size, 4))
        # Generate data
        ind = 0
        for _, row in tmp_df.iterrows():
            ...
            bboxes = self.__get_bboxes(row['name'])

            ratio = 0.45
            l_score = np.zeros(self.target_size, np.float32)
            l_bbox = np.zeros((*self.target_size, 4), np.float32)
            ...
            for bbox in bboxes:
                ...
                x1 = np.maximum(x1, 0)
                y1 = np.maximum(y1, 0)
                x2 = np.minimum(x1+w, self.target_size[1])
                y2 = np.minimum(y1+h, self.target_size[0])
            ...
            l_scores[ind,:,:,0] = msk_circle
            l_bboxes[ind,:,:,:] = l_bbox.reshape(*self.target_size,4)
            ind += 1
